function* servers(ns: IGame): IterableIterator<string> {
  let queue: string[] = [];
...
function* contracts(ns: IGame): IterableIterator<LocatedContract> {
...
function* primeGen() {
  ...
  for (let i = 13; ; i += 2) {
    if (primes.some(p => (i % p) === 0)) continue;
    primes.push(i);
    yield i;
  }
}

async function largestPrimeFactor(ns: IGame, n: number) {
  let primes = primeGen()
  let remainder = n;
...
function generateTrailingIPComponents(s: string, depth: number, soFar: string): string[] {
  if (depth == 5) {
...
function generateIPAddresses(n: number) {
  return generateTrailingIPComponents(n.toString(), 1, "");
}

function algoStockTrader(prices: number[], remainingTransactions: number): number {
  ...
  for (let i = 0; i < prices.length; ++i) {
    ...
    if (thisTrade > bestTrade) bestTrade = thisTrade;
  }
  return bestTrade;
}

function mergeOverlapping(n: Array<Array<number>>) {
  n.sort((x, y) => x[0] - y[0]);
...
async function solveContract(ns: IGame, contract: LocatedContract) {
  ...
  if (type == "Find Largest Prime Factor") {
    ...
    ns.write(
      "contract_log.txt",
      `unhandled coding contract ${nameForDbg}: ${type}\n`);
    return;
  }
...
async function solveAllPendingContracts(ns: IGame) {
